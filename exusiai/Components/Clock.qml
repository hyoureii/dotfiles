...
Column {
    ...
    Label {
        ...
        renderType: Text.QtRendering
        function updateTime() {
            text = "Exu says it's " + new Date().toLocaleDateString(Qt.locale(config.Locale), config.DateFormat == "short" ? Locale.ShortFormat : config.DateFormat !== "" ? config.DateFormat : Locale.LongFormat)
        }
    }
